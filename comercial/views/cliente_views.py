...
def lista_clientes(request):
    # Base geral sem filtros para os indicadores
    clientes_all = Cliente.objects.all()

    # Parâmetros GET
    razao_social  = request.GET.get('razao_social', '')
    cnpj          = request.GET.get('cnpj', '')
    status        = request.GET.get('status', 'Ativo')  # padrão
    tipo_cadastro = request.GET.get('tipo_cadastro', 'Cliente')  # padrão

    # Aplicação de filtros para a listagem
    qs = clientes_all
    if razao_social:
    ...
    # Indicadores globais
    total_clientes   = clientes_all.count()
    total_ativos     = clientes_all.filter(status='Ativo').count()
    total_inativos   = clientes_all.filter(status='Inativo').count()
    mes_atual        = timezone.now().month
    atualizadas_mes  = clientes_all.filter(atualizado_em__month=mes_atual).count()
    ...
    # Paginação da listagem filtrada
