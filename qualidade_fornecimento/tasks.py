...
from qualidade_fornecimento.services.pdf_inspecao_servico_externo import (
...
def gerar_pdf_inspecao_servico_background(servico_id: int):
    ...
    caminho = gerar_pdf_inspecao_servico_externo(
        servico
    )  # Precisa que o service retorne o caminho do arquivo
        ...
        if os.path.exists(full_path):
            with open(full_path, "rb") as fp:
                servico.inspecao.pdf.save(
                    os.path.basename(full_path), File(fp), save=True
                )
