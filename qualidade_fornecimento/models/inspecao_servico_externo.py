...
class InspecaoServicoExterno(models.Model):
    servico = models.OneToOneField(
        ControleServicoExterno, on_delete=models.CASCADE, related_name="inspecao"
    )
    certificado_numero = models.CharField(max_length=100)
    certificado_anexo = models.FileField(
        upload_to="certificados_servico_externo/", blank=True, null=True
    )

    inspecao_visual = models.CharField(
        max_length=10, choices=[("OK", "OK"), ("NOK", "NOK")]
    )
    espessura_camada = models.CharField(
        max_length=10, choices=[("OK", "OK"), ("NOK", "NOK")]
    )
