...
def listar_materia_prima_catalogo(request):
    ...
    normas_disponiveis = MateriaPrimaCatalogo.objects.exclude(norma__isnull=True).exclude(norma__exact='').values_list('norma', flat=True).distinct().order_by('norma')
        ...
        try:
            ...
            for col in colunas_obrigatorias:
                if col not in df.columns:
                    messages.error(request, f"A coluna obrigatória '{col}' não está presente.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'salvar_proximo' in request.POST:
                ...
                proxima = MateriaPrimaCatalogo.objects.filter(id__gt=materia_prima.id).order_by('id').first()
                if proxima:
