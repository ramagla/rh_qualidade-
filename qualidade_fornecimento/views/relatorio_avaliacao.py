    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tipo == 'servico':
                ...
                reprovados = sum(1 for d in dados if d.status_geral() in ["Reprovado", "Aprovado Condicionalmente"])
                atrasos = [
                    s.servico.atraso_em_dias for s in dados
                    if hasattr(s.servico, 'atraso_em_dias') and s.servico.atraso_em_dias is not None
                ]
            else:
                dados = RelacaoMateriaPrima.objects.filter(
                    fornecedor=fornecedor,
                    data_entrada__range=[inicio, fim]
                )
                reprovados = dados.filter(status__in=["Reprovado", "Aprovado Condicionalmente"]).count()
            ...
            caminho_absoluto = os.path.join(settings.MEDIA_ROOT, 'graficos', nome_arquivo)
            caminho_relativo = os.path.join(settings.MEDIA_URL, 'graficos', nome_arquivo)
        ...
        form = RelatorioAvaliacaoForm(initial={
            'ano': ano_atual,
            'semestre': semestre_atual
        })

    return render(request, 'relatorios/relatorio_avaliacao.html', {'form': form, 'exibir': False})
