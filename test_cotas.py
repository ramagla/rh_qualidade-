...
for key, value in data.items():
    if key.startswith('afericoes['):
        ...
        if cota:
            ...
            status = 'Aprovado' if cota.valor_minimo <= valor <= cota.valor_maximo else 'Reprovado'
            print(f" - Status: {status}")
        else:
            print(f"Erro: Cota {cota_numero} não encontrada para o dispositivo {dispositivo.codigo}")
