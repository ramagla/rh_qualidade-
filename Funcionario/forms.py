...
from .models import Documento, IndicadorDesempenho, PlanoAcaoMelhoria, RevisaoDoc
...
class FuncionarioForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            "status",
            "formulario_f146",
            "experiencia_profissional",  # Inclua o novo campo aqui
        ]

    def __init__(self, *args, **kwargs):
        super(FuncionarioForm, self).__init__(*args, **kwargs)
        for field in self.fields:
            # Evita sobrescrever widgets Select2
...
class AvaliacaoTreinamentoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if funcionario_id:
            self.fields["treinamento"].queryset = Treinamento.objects.filter(
...
class JobRotationEvaluationForm(forms.ModelForm):
    ...
    def clean_data_inicio(self):
        ...
        if (
            data_inicio and data_inicio < timezone.now().date()
        ):  # Caso queira validar se a data é futura
